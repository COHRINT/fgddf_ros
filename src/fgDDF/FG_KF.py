...
class FG_KF(object):
	...
	def __init__(self, variables, varSet, measurementData, uData, dt ):
		"""
		...
		for var in agent.varSet:
			if var in self.dynamicList:
				...
				try:
					...
					predVec_k=-np.dot(np.dot(np.dot(F.T, Qinv), G), u)  # current time step
				except KeyError:
				...
				f_k=nodes.FNode("f_"+str(factorCounter),
								rv.Gaussian.inf_form(predMat_k, predVec_k, *instances_k))
				factorCounter=factorCounter+1

				f_kp1_k=nodes.FNode("f_"+str(factorCounter),
									rv.Gaussian.inf_form(predMat_kp1_k, np.zeros([2*n, 1], dtype=np.float64), *instances_kp1_k))
				factorCounter=factorCounter+1

				f_kp1=nodes.FNode("f_"+str(factorCounter),
				...
				x=[]
				for i in range(len(list_vnodes)):  # You can display the name of all nodes
		...
		for key in self.currentMeas:
			...
			for var in self.measurementData[key]['measuredVars']:
				...
				f_i=nodes.FNode("f_"+str(factorCounter),
								rv.Gaussian.inf_form(measMat[varDict[var][0]:varDict[var][-1]+1, varDict[var][0]:varDict[var][-1]+1],
													 measVec[varDict[var][0]:varDict[var][-1]+1], *varList[varDict[var][0]:varDict[var][-1]+1]))
				tmpMeasMat[varDict[var][0]:varDict[var][-1]+1, varDict[var][0]:varDict[var][-1]+1] = 0
				factorCounter+=1
		...
		for var in nodesToMarginalize:
			...
			if factorFlag:
				...
				if len(tmpfactor.dim) > 0:
					...
					tmpFnode=nodes.FNode("f_"+str(factorCounter),
										 rv.Gaussian.inf_form(tmpfactor._W, tmpfactor._Wm, *tmpfactor.dim))
...
class FG_EKF(FG_KF):
	...
	def add_Prediction(self, agent):
		""" Add nonlinear Gaussian prediction factor and new prediction nodes
            works in canonical / information form
            provision for using a different linearization point
        Args:
        	x_m - prior linearization point
        """
		dt = self.dt
		...
		for var in agent.varSet:
			if var in self.dynamicList:
				...
				predVec_kp1 = np.dot(Qinv, dx)  # next time step
				predVec_k = -np.dot(np.dot(F.T, Qinv), dx)  # current time step
	...
	def add_Measurement(self, agent):
		""" Add linear Gaussian measurement factor
            works in canonical / information form
        Args:
            self.currentMeas: a dictionary containing current time step measurements:  y
            x_p - posterior linearization point
        """
		factorCounter=agent.factorCounter
		#for key in self.measurementData:
		for key2 in self.currentMeas:

			Htmp = self.measurementData[key2[0]]['measJacobian'](self, key2)
			hx = self.measurementData[key2[0]]['measFxn'](self, key2)

			x_p = self.x_hat[self.measurementData[key2[0]]['measuredVars'][key2[0]][0]]
			for ii in range(1, len(self.measurementData[key2[0]]['measuredVars'][key2[1]])):
				if self.measurementData[key2[0]]['measuredVars'][key2[1]][ii] in agent.varSet:
					x_p = np.append(x_p, self.x_hat[self.measurementData[key2[0]]['measuredVars'][key2[1]][ii]], axis=0)

			#jointInfMat=buildJointMatrix(agent)
			# H = np.stack((H1,H2))
			...
			if key2[0]==1:
					#Htag = deepcopy(H)
					#Htag[0,2] = 0
					#correct_angle = correctQuadrant(np.dot(Htag,x_p), x_p[4,0]-x_p[1,0], x_p[3,0]-x_p[0,0])
					#dy = dy-np.dot(Htag,x_p)+correct_angle
				dy = wrapToPi(dy)


			measVec=np.dot(np.dot(H.T, invR), dy)
			tmpMeasMat = deepcopy(measMat)
			...
			l = 0
			for var in self.measurementData[key2[0]]['measuredVars'][key2[1]]:
				if var in agent.varSet:
					X=getattr(self, var)
					n=X['n']
					varDict[var] = []
					for i in range(n):
			...
			if nV>1:
				...
				agent=mergeFactors(agent, self.measurementData[key2[0]]['measuredVars'][key2[1]])
			else:
				agent=mergeFactors(agent, self.measurementData[key2[0]]['measuredVars'][key2[1]][0])
