...
# SIM TARGET CODE
def sim_target1_callback(msg):
    if msg.target == target1: 
        sim_target1_pos[msg.time_step - 1] = msg.position
def sim_target2_callback(msg):
    if msg.target == target2:
        sim_target2_pos[msg.time_step - 1] = msg.position
def sim_target3_callback(msg):
    if msg.target == target3:
        sim_target3_pos[msg.time_step - 1] = msg.position
def sim_target4_callback(msg):
    if msg.target == target4:
        sim_target4_pos[msg.time_step - 1] = msg.position
def sim_target5_callback(msg):
    if msg.target == target5:
        sim_target5_pos[msg.time_step - 1] = msg.position
def sim_target6_callback(msg):
    if msg.target == target6:
        sim_target6_pos[msg.time_step - 1] = msg.position
def sim_target7_callback(msg):
    if msg.target == target7:
        sim_target7_pos[msg.time_step - 1] = msg.position
def sim_target8_callback(msg):
    if msg.target == target8:
        sim_target8_pos[msg.time_step - 1] = msg.position
def sim_target9_callback(msg):
    if msg.target == target9:
        sim_target9_pos[msg.time_step - 1] = msg.position
def sim_target10_callback(msg):
    if msg.target == target10:
        sim_target10_pos[msg.time_step - 1] = msg.position

def sim_agent1_callback(msg):
    if msg.target == agent1:
        sim_agent1_pos[msg.time_step - 1] = msg.position
def sim_agent2_callback(msg):
    if msg.target == agent2:
        sim_agent2_pos[msg.time_step - 1] = msg.position
def sim_agent3_callback(msg):
    if msg.target == agent3:
        sim_agent3_pos[msg.time_step - 1] = msg.position
...
if len(sim_targets):
    ...
    for st in sim_targets:
        if st == target1:
            sim_target1_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target1_callback)
            sim_target1_pos = np.empty([200,3])
        if st == target2:
            sim_target2_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target2_callback)
            sim_target2_pos = np.empty([200,3])
        if st == target3:
            sim_target3_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target3_callback)
            sim_target3_pos = np.empty([200,3])
        if st == target4:
            sim_target4_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target4_callback)
            sim_target4_pos = np.empty([200,3])
        if st == target5:
            sim_target5_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target5_callback)
            sim_target5_pos = np.empty([200,3])
        if st == target6:
            sim_target6_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target6_callback)
            sim_target6_pos = np.empty([200,3])
        if st == target7:
            sim_target7_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target7_callback)
            sim_target7_pos = np.empty([200,3])
        if st == target8:
            sim_target8_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target8_callback)
            sim_target8_pos = np.empty([200,3])
        if st == target9:
            sim_target9_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target9_callback)
            sim_target9_pos = np.empty([200,3])
        if st == target10:
            sim_target10_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_target10_callback)
            sim_target10_pos = np.empty([200,3])

if len(sim_agents):
    for sa in sim_agents:
        if sa == agent1:
            sim_agent1_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_agent1_callback)
            sim_agent1_pos = np.empty([200,3])
        if sa == agent2:
            sim_agent2_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_agent2_callback)
            sim_agent2_pos = np.empty([200,3])
        if sa == agent3:
            sim_agent3_sub = rospy.Subscriber("sim_truth_data",TruthData,sim_agent3_callback)
            sim_agent3_pos = np.empty([200,3])
...
k = 2
rospy.sleep(1)
while not rospy.is_shutdown() and (k < 200): # This is why we don't get measurements for time step 200
    # SIM TARGET CODE
    # Replay simulated target position measurements
    for st in sim_targets:
        if st == target1:
            msg = PoseStamped()
            msg.pose.position.x = sim_target1_pos[k-1,0]
            msg.pose.position.y = sim_target1_pos[k-1,1]
            msg.pose.position.z = sim_target1_pos[k-1,2]
            pub_sim_target1_pose.publish(msg)
        if st == target2:
            msg = PoseStamped()
            msg.pose.position.x = sim_target2_pos[k-1,0]
            msg.pose.position.y = sim_target2_pos[k-1,1]
            msg.pose.position.z = sim_target2_pos[k-1,2]
            pub_sim_target2_pose.publish(msg)
        if st == target3:
            msg = PoseStamped()
            msg.pose.position.x = sim_target3_pos[k-1,0]
            msg.pose.position.y = sim_target3_pos[k-1,1]
            msg.pose.position.z = sim_target3_pos[k-1,2]
            pub_sim_target3_pose.publish(msg)
        if st == target4:
            msg = PoseStamped()
            msg.pose.position.x = sim_target4_pos[k-1,0]
            msg.pose.position.y = sim_target4_pos[k-1,1]
            msg.pose.position.z = sim_target4_pos[k-1,2]
            pub_sim_target4_pose.publish(msg)
        if st == target5:
            msg = PoseStamped()
            msg.pose.position.x = sim_target5_pos[k-1,0]
            msg.pose.position.y = sim_target5_pos[k-1,1]
            msg.pose.position.z = sim_target5_pos[k-1,2]
            pub_sim_target5_pose.publish(msg)
        if st == target6:
            msg = PoseStamped()
            msg.pose.position.x = sim_target6_pos[k-1,0]
            msg.pose.position.y = sim_target6_pos[k-1,1]
            msg.pose.position.z = sim_target6_pos[k-1,2]
            pub_sim_target6_pose.publish(msg)
        if st == target7:
            msg = PoseStamped()
            msg.pose.position.x = sim_target7_pos[k-1,0]
            msg.pose.position.y = sim_target7_pos[k-1,1]
            msg.pose.position.z = sim_target7_pos[k-1,2]
            pub_sim_target7_pose.publish(msg)
        if st == target8:
            msg = PoseStamped()
            msg.pose.position.x = sim_target8_pos[k-1,0]
            msg.pose.position.y = sim_target8_pos[k-1,1]
            msg.pose.position.z = sim_target8_pos[k-1,2]
            pub_sim_target8_pose.publish(msg)
        if st == target9:
            msg = PoseStamped()
            msg.pose.position.x = sim_target9_pos[k-1,0]
            msg.pose.position.y = sim_target9_pos[k-1,1]
            msg.pose.position.z = sim_target9_pos[k-1,2]
            pub_sim_target9_pose.publish(msg)
        if st == target10:
            msg = PoseStamped()
            msg.pose.position.x = sim_target10_pos[k-1,0]
            msg.pose.position.y = sim_target10_pos[k-1,1]
            msg.pose.position.z = sim_target10_pos[k-1,2]
            pub_sim_target8_pose.publish(msg)

    for sa in sim_agents:
        if sa == agent1:
            msg = PoseStamped()
            msg.pose.position.x = sim_agent1_pos[k-1,0]
            msg.pose.position.y = sim_agent1_pos[k-1,1]
            msg.pose.position.z = sim_agent1_pos[k-1,2]
            pub_sim_agent1_pose.publish(msg)
        if sa == agent2:
            msg = PoseStamped()
            msg.pose.position.x = sim_agent2_pos[k-1,0]
            msg.pose.position.y = sim_agent2_pos[k-1,1]
            msg.pose.position.z = sim_agent2_pos[k-1,2]
            pub_sim_agent2_pose.publish(msg)
        if sa == agent3:
            msg = PoseStamped()
            msg.pose.position.x = sim_agent3_pos[k-1,0]
            msg.pose.position.y = sim_agent3_pos[k-1,1]
            msg.pose.position.z = sim_agent3_pos[k-1,2]
            pub_sim_agent3_pose.publish(msg)
